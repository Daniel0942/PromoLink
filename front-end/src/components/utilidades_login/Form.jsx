...
function Form({ txtTitulo, txt1, txt2, type1, type2, txtButton, input_user, txt_P1, txt_P2, rota_link, requisisao }) {
    ...
    async function Logar(e) {
        ...
        if (email && password) {
            ...
            try {
                ...
                // Armazenar o token e refresh_token no localStorage
    ...
    return (
        <>
            ...
            <form action="#" className={style.form} onSubmit={requisisao === "logar" ? Logar : Registrar}>
                {/*input do user da página de registrar*/}
                <div
                    className={`${style.caixa_input} 
                ...
                <div className={style.caixa_input}>
                    ...
                    <input type={type1} placeholder={txt1} onChange={(e) => setEmail(e.target.value)} />
                </div>
                <div className={style.caixa_input}>
                    <img src={lock} alt="cadeado em svg do figma" />
                    <input type={type2} placeholder={txt2} onChange={(e) => setPassword(e.target.value)} />
                </div>

                <p>
                    {txt_P1} <Link to={rota_link}>{txt_P2}</Link>
                </p>
