...
function Principal() {
    ...
    async function EnviarSite(e) {
        ...
        if (site.length > 0) {
            ...
            await axios
                .post(
                    `http://127.0.0.1:5000/produtos/${site}`,
                    {},
                    {
    ...
    function EnviarSiteHistorico() {
        axios
            ...
            .catch((erro) => {
                console.error(`Ocorreu ao enviar o siteHistorico: ${erro}`);
            });
    }
    ...
    async function PesquisarProduto() {
        if (pesquisa) {
            ...
            try {
                ...
                setCarregamento(false);
                let msg =
                    response.data.Success || "Falha na conexão com o servidor";
                showMessage(msg, "sucess");
                navigate("/principal/pesquisa", {
                    state: { username, pesquisa },
                });
            } catch (err) {
                setCarregamento(false);
